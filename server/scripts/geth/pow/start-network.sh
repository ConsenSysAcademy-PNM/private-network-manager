...
do
  ...
  screen -dmS "Node-$counter" geth --datadir ./server/networks/${NETWORK_ID}/node-${counter} --unlock "$BASE_ADDRESS" --password "./server/scripts/geth/password.txt" --networkid $NETWORK_ID --port $port --bootnodes enode://$(cat ./server/networks/${NETWORK_ID}/enode)@127.0.0.1:30301 "$MINE" --rpc --rpcapi "eth,net,web3"
